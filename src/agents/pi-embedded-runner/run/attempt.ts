...
const CODING_ENHANCE_RE = /(?:^|\s)\/coding\s+enhance(?:\s|$)/i;
const CODING_ENHANCE_STRIP_RE = /(^|\s)\/coding\s+enhance(?=\s|$)/gi;
const CODING_ENHANCE_SYSTEM_PROMPT =
  "When the user's current request included `/coding enhance`, apply best coding practices for any implementation work: keep changes clear, maintainable, and consistent with local code patterns; prefer readable structure and safe incremental edits; and add or improve comments where they materially help future readers understand intent, non-obvious logic, assumptions, or edge cases without turning the code into comment spam. For shell/exec work, do not blindly execute a long compound command generated by the model when a safer sequence of smaller commands would work. Prefer splitting independent steps into separate commands so approval, auditing, failure isolation, and retries are simpler. Only keep commands combined when they truly need shared shell state or shell operators for correctness. Also, if you create or modify files while completing that request, your next user-facing reply must include a short explicit list of the created or modified file paths. Use workspace-relative paths when practical. If no files were created or modified, do not add an empty list or mention this rule.";

export function resolveCodingEnhancePrompt(prompt: string): { enabled: boolean; prompt: string } {
  const raw = String(prompt ?? "");
  const enabled = CODING_ENHANCE_RE.test(raw);
  if (!enabled) {
    return { enabled: false, prompt: raw };
  }
  const stripped = raw
    .replace(CODING_ENHANCE_STRIP_RE, "$1")
    .replace(/\n{3,}/g, "\n\n")
    .replace(/[ \t]{2,}/g, " ")
    .trim();
  return {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        const codingEnhance = resolveCodingEnhancePrompt(params.prompt);
        let effectivePrompt = prependBootstrapPromptWarning(
          codingEnhance.prompt,
        ...
        });
        {
          ...
          const prependedOrAppendedSystemPrompt = composeSystemPromptWithHookContext({
            ...
            appendSystemContext: joinPresentTextSegments([
              ...
              codingEnhance.enabled ? CODING_ENHANCE_SYSTEM_PROMPT : undefined,
            ]),
          });
